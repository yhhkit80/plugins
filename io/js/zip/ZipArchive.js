...
var ZipArchive = function() {
	...
	this.onend = noop;
	this.onerror = noop;
	this.onprogress = noop; //创建一个延迟对象;
	var def = this.defer = new $.Deferred();
	zip.createWriter(new zip.BlobWriter("application/zip"), function(zipWriter) {
		_this.zipWriter = zipWriter; //继续执行队列;
...
$.extend(ZipArchive.prototype, {
	...
	"addFile": function(filename, content, options) {
		var _this = this;
		var blob = content;
		//blob = ZipArchive.blob(filename, content); //为了产生链式的效果， 必须把deferrer赋值给新的defer

		this.defer = this.defer.then(function() {
			var def = $.Deferred();
			_this.zipWriter.add(filename, new zip.BlobReader(blob), function() { // reader

				console.log("addFile success!!");
				def.resolve(); //zipWriter.close(callback);
	 ...
	 * */
	"_addFolder": function(foldername, options) { //创建文件夹功能目前不能用;
			...
			_this.zipWriter.close(function(zippedBlob) {
				if(typeof name === "string" || typeof name === "number") {
					if(typeof navigator.msSaveBlob == "function") {
						navigator.msSaveBlob(zippedBlob, name + ".zip");
					} else {
